...
def task1():
    ...
    loader_train = DataLoader(dataset_train, batch_size=8,shuffle=True,pin_memory=True,num_workers=4)
    dataset_test = ChallengeDataset('dataset','test',rep_type)
    loader_test = DataLoader(dataset_test, batch_size=8,shuffle=True,pin_memory=True,num_workers=4)
...
def task2():
    ...
    if os.path.isfile(path_model):
        ...
        net = voxel_model(NUM_CLASSES)
        # print(net)# voxel_model(n) or point_model(n) or spectral_model(n)
        print('Training from scratch')
